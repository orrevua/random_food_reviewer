...
export default function ReviewRow({
  ...
  const name = review.profile?.display_name ?? (isMine ? 'Você' : 'Usuário')

  return (
    <div className="review-row">
      <div className="review-head">
        <span className="reviewer-name">{name}{isMine ? ' (você)' : ''}</span>
      ...
      {sorted.length > 0 && (
        <ul className="review-topics">
          {sorted.map((rr) => {
            const t = topicsById.get(rr.topic_id)
            return (
              <li key={rr.topic_id}>
                {t?.label ?? '(tópico removido)'}: {rr.score}/10
              </li>
            )
          })}
        </ul>
